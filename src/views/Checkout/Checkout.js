...
const Checkout = () => {
    ...
    const { validateForm, validateInputsForm, isFormValid } = useCheckoutContext()
    ...
    const createPreference = async () => {
        ...
        const rawOrderId = await fetch('/api/createPreference', {
            method: 'POST',
            headers: {
    ...
    const checkForm = () => {
        const modal = document.getElementsByClassName('mp-mercadopago-checkout-wrapper')
    ...
    return (
        <div className="Checkout">            
            ...
            <div className="Checkout__pay-buttons">
                <button className="Checkout__pay-button Checkout__pay-button-2" style={{ display: validateForm() ? "none" : "flex" }} onClick={checkForm}>Pagar</button>
                <div id={FORM_ID} className="Checkout__pay-button" style={{ display: validateForm() ? "flex" : "none" }} onClick={checkForm}/>
            </div>
        </div>
    );
}
